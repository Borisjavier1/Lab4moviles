...
class CreateTeacherFragment : FragmentUtils() {
    ...
    override fun onCreateView(
        ...
        if(sentPerson != null){
            ...
            profesor = sentPerson as ProfesorAPIItem
    ...
    private fun createPerson(){
        ...
        if(tipoAgregado == 0){
           ...
           post(cedula,email,guia.toInt(),nombre,tele)
            ...
            put(profesor.id,cedula,email,guia.toInt(),nombre,tele)
        }
        /*Snackbar
    ...
    fun post(cedula: String,email: String,guia:Int,nombre:String,telefono:String) {

        val jsonObject = JSONObject()
        jsonObject.put("cedula",cedula)
        jsonObject.put("email",email)
        jsonObject.put("guia",guia)
        jsonObject.put("id",99)
        jsonObject.put("nombre",nombre)
        jsonObject.put("telefono",telefono)

        val request = JsonObjectRequest(
            Request.Method.POST,getString(R.string.url)+"insertarProfesor",jsonObject,
            { response->
                // Process the json
                try {
                ...
                if(it.message?.contains("true") == true){
                    Toast.makeText(activity, "Profesor insertado.", Toast.LENGTH_LONG).show()
    ...
    @SuppressLint("SetTextI18n")
    fun put(id:Int, cedula: String,email: String,guia:Int,nombre:String,telefono:String) {
        val jsonObject = JSONObject()
        jsonObject.put("cedula",cedula)
        jsonObject.put("email",email)
        jsonObject.put("guia",guia)
        ...
        val request = JsonObjectRequest(Request.Method.PUT,getString(R.string.url)+"actualizarProfesor/"+id,jsonObject,
            { response ->
                // Process the json
                try {
                    // etCord.setText("Response: $response")
                ...
                if(it.message?.contains("true") == true){
                    Toast.makeText(activity, "Profesor modificado.", Toast.LENGTH_LONG).show()
                }
            })
